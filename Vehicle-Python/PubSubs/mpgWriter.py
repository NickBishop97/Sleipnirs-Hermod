...
def main():
    ...
    readers.append(
        FuelGauge([Fuel, "Fuel", "FuelRemaining544645", FuelRL])
    )  # noqa: F405
    readers.append(
        DistanceDisplay([Miles, "Miles", "MilesTraveled", DistanceRL])
    )  # noqa: F405

    writers.append(
        MpGWriter([MpG, "MpG", "MpGCumulative"])
    )  # noqa F405 (linting exemption)
